...
void IndirectSpecularLightingRenderPass::Execute(ID3D12GraphicsCommandList *commandList, D3D12_CPU_DESCRIPTOR_HANDLE *depthStencilViewPtr,
...
void IndirectSpecularLightingRenderPass::Draw(ID3D12GraphicsCommandList * commandList, ID3D12Resource * objectCB, ID3D12Resource * matCB)
{
	SceneManager::GetScenePtr()->mQuadrObject->Draw(commandList, nullptr, nullptr, mSrvDescriptorHeap, 0, 0, 0, false);
}

void IndirectSpecularLightingRenderPass::BuildRootSignature()
{
	// 4 GBuffers + 1 Main Texture + 2 Voxel Grids
...
void IndirectSpecularLightingRenderPass::BuildDescriptorHeaps()
{
	...
	md3dDevice->CreateShaderResourceView(mVoxelGrids[0].Get(), &srvDescVoxelGrid, hDescriptor);

	hDescriptor.Offset(1, cbvSrvDescriptorSize);

	md3dDevice->CreateShaderResourceView(mVoxelGrids[2].Get(), &srvDescVoxelGrid, hDescriptor);
}

void IndirectSpecularLightingRenderPass::BuildPSOs()
{
